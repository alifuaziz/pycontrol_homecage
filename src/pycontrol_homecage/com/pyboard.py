...
class Pyboard:
    def __init__(self, serial_device, baudrate=115200):
        self.serial = serial.Serial(
            serial_device, baudrate=baudrate, interCharTimeout=1
        )
    ...
    def write_file(self, target_path, data):
        ...
        try:
            self.exec(
                "with open('{}','w') as f: f.write({})".format(target_path, repr(data))
            )
        except PyboardError as e:
        ...
        try:
            for i in range(10):
                ...
                self.exec_raw_no_follow(
                    "_receive_file('{}',{})".format(target_path, file_size)
                )
                with open(file_path, "rb") as f:
                    while True:
                        chunk = f.read(512)
    ...
    def transfer_folder(
        self, folder_path, target_folder=None, file_type="all", show_progress=False
    ):
