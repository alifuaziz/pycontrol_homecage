...
class Data_logger:
    ...
    def set_state_machine(self, sm_info: dict):
        ...
        self.ID2name_fw = self.sm_info[
            "ID2name"
        ]  # Dict mapping framework IDs to names.
        self.ID2name_hw = {
            ai["ID"]: name
            for name, ai in self.sm_info[  # Dict mapping hardware IDs to names.
                "analog_inputs"
            ].items()
        }
        self.analog_files = {
            ai["ID"]: None for ai in self.sm_info["analog_inputs"].values()
        }
    ...
    def open_data_file(
        ...
        file_name = os.path.join(
            self.subject_ID + datetime_now.strftime("-%Y-%m-%d-%H%M%S") + ".txt"
        )
        ...
        self.data_file.write(
            "I Start date : " + datetime_now.strftime("%Y/%m/%d %H:%M:%S") + "\n\n"
        )
    ...
    def save_analog_chunk(self, ID, sampling_rate, timestamp, data_array):
        ...
        if not self.analog_files[ID]:
            file_name = (
                os.path.splitext(self.file_path)[0] + "_" + self.ID2name_hw[ID] + ".pca"
            )
