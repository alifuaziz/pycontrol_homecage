...
class ExperimentOverviewTable(QtWidgets.QTableWidget):
    ...
    def end_experiments(self, experiment_names: List[str]) -> None:
        ...
        for exp_name in experiment_names:
            for setup in database.exp_df.loc[
                database.exp_df["Name"] == exp_name, "Setups"
            ].values:
                setup = eval(setup)[0]
                ...
                if database.controllers.items():  # if there are any controllers
                    ...
                    handler_ = [
                        setup_
                        for k, setup_ in database.controllers.items()
                        if k == setup
                    ][0]
                    ...
                    database.exp_df.loc[
                        database.exp_df["Name"] == exp_name, "Active"
                    ] = False
                ...
                for subject in eval(
                    database.exp_df.loc[
                        database.exp_df["Name"] == exp_name, "Subjects"
                    ].values[0]
                ):
                    database.mouse_df.loc[
                        database.mouse_df["Mouse_ID"] == subject, "in_system"
                    ] = False
