...
class Run_task_tab(QtWidgets.QWidget):
    ...
    def configure_board(self, index):
        if index:
            ...
            if index == 1:
                ...
                flashdrive_enabled = "MSC" in self.pyc_board.status["usb_mode"]
                ...
                if hwd_path:
                    self.pyc_board.load_hardware_definition(hwd_path)
            elif index == 3:
                self.config_dropdown.setCurrentIndex(0)
            ...
            if disconnect:
                ...
                self.disconnect()
            if self.connected and self.pyc_board.status["framework"]:
    ...
    def connect(self):
        ...
        try:
            ...
            self.connect_button.setEnabled(False)
            self.repaint()
            self.pyc_serial_port = self.GUI_main.setups_tab.get_port(self.board_select.currentText())
            self.pyc_board = Pycboard(
                self.pyc_serial_port, print_func=self.print_to_log, data_consumers=[self.task_plot, self.task_info]
            )
            self.ac_serial_port = self.GUI_main.setups_tab.get_port(self.board_select.currentText(), pyc_board=False)
            self.ac_board = Access_control(self.ac_serial_port, print_func=self.print_to_log)
            self.connected = True
            ...
            if self.pyc_board.status["framework"]:
                self.task_groupbox.setEnabled(True)
            else:
    ...
    def setup_task(self):
        ...
        try:
            ...
            self.controls_button.setEnabled(False)
            self.repaint()
            self.pyc_board.setup_state_machine(task, uploaded=self.uploaded)
            self.initialise_API()
            self.task = task
            # Set values for any hardware variables.
            task_hw_vars = [task_var for task_var in self.pyc_board.sm_info.variables if task_var.startswith("hw_")]
            if task_hw_vars:
            ...
            if "custom_controls_dialog" in self.pyc_board.sm_info.variables:
                custom_variables_name = self.pyc_board.sm_info.variables["custom_controls_dialog"]
                ...
                if potential_dialog.custom_gui == Custom_gui.JSON:
                    ...
                    self.controls_dialog = py_gui_file.Custom_controls_dialog(self, self.pyc_board)
            self.controls_button.clicked.connect(self.controls_dialog.exec)
            self.controls_button.setEnabled(True)
            self.task_plot.set_state_machine(self.pyc_board.sm_info)
            self.task_info.set_state_machine(self.pyc_board.sm_info)
    ...
    def initialise_API(self):
        ...
        self.pyc_board.data_consumers = [self.task_plot, self.task_info]
        if "api_class" not in self.pyc_board.sm_info.variables:
            return  # Task does not use API.
        API_name = self.pyc_board.sm_info.variables["api_class"]
        ...
        try:
            ...
            self.user_API.interface(self.pyc_board, self.print_to_log)
            self.pyc_board.data_consumers.insert(0, self.user_API)
    ...
    def start_task(self):
        ...
        if recording:
            ...
            self.pyc_board.data_logger.open_data_file(self.data_dir, "run_task", setup_ID, subject_ID)
            self.pyc_board.data_logger.copy_task_file(
                self.data_dir, self.GUI_main.task_directory, "run_task-task_files"
            )
        self.fresh_task = False
        self.running = True
        self.pyc_board.start_framework()
    ...
    def stop_task(self, error=False, stopped_by_task=False):
        ...
        if not (error or stopped_by_task):
            self.pyc_board.stop_framework()
            time.sleep(0.05)
            try:
                self.pyc_board.process_data()
                self.print_to_log(f"\nRun stopped at: {datetime.now().strftime('%Y/%m/%d %H:%M:%S')}")
            except PyboardError:
                self.print_to_log("\nError while stopping framework run.")
        self.pyc_board.data_logger.close_files()
    ...
    def plot_update(self):
        ...
        try:
            self.pyc_board.process_data()
            if not self.pyc_board.framework_running:
